"""
GSPO v32 — Layered refinement on top of v31 GRPO adapter
- Loads /workspace/output/nemotron-lora-adapter (the v31 GRPO result)
- Uses Group SEQUENCE Policy Optimization (importance_sampling_level="sequence")
  instead of token-level (default GRPO). GSPO is more stable for long traces.
- 50 balanced samples (28 binary, 12 cipher, 4 gravity, 3 numeral, 2 unit, 1 equation)
- Only 2 steps — this is a polish, not a re-train
- save_steps=1 so we capture every step
- Output written to /workspace/output_v32/ to keep separate from v31
"""
...
# Load v31 GRPO adapter as the starting point (NOT v22)
ADAPTER_IN = os.path.join(WORKSPACE, "output", "nemotron-lora-adapter")
DATA_PATH = os.path.join(WORKSPACE, "data", "gspo_50.jsonl")
OUTPUT_DIR = os.path.join(WORKSPACE, "output_v32")
...
# GSPO config — 2-step polish
# importance_sampling_level="sequence" turns GRPO into GSPO
GRPO_LR = 5e-6
GRPO_STEPS = 2          # only 2 steps for polish
GRPO_GENERATIONS = 2
GRPO_BATCH = 1
GRPO_GRAD_ACCUM = 4
GRPO_TEMP = 0.9
...
GRPO_MAX_PROMPT = 512
GRPO_SAVE_STEPS = 1     # save every step
...
print(f"Loading v31 GRPO adapter from {ADAPTER_IN} ...")
...
print("v31 GRPO adapter loaded. Now doing GSPO refinement on top.")
...
training_args = GRPOConfig(
    ...
    seed=RANDOM_SEED,
    loss_type="grpo",
    importance_sampling_level="sequence",  # GSPO instead of GRPO (sequence-level ratios)
)

print(f"\n{'='*60}")
print(f"GSPO v32 (RunPod) — v31 GRPO adapter + 50 balanced samples, max_steps={GRPO_STEPS}, save_steps={GRPO_SAVE_STEPS}")
print(f"  importance_sampling_level=sequence (GSPO mode)")
print(f"{'='*60}")
