"""
SFT v30 — RunPod adaptation
MIXED DATA: v22 verbose CoT (504) + v29 Donald templated (500) = 1004 examples.
Same proven v22 config (r=32, alpha=64, LR=2e-4, 3 epochs, 4 LoRA targets).

This is the Stage-1 fix per the v33 GRPO diagnosis:
- v33 showed GRPO can't create behaviors the base model doesn't sample
- v22 SFT never produced labeled steps → GRPO couldn't reinforce pipeline_step rewards
- v30 SFT teaches the labeled-step structure via mixed examples
- Then GRPO on v30 should actually amplify the right behavior

Layout on pod:
  /workspace/
    train_sft_v30.py       (this file)
    data/training_v30.jsonl (downloaded via kaggle CLI from bharatmohan/nemotron-training-v30)
    model/                  (Nemotron-3-Nano-30B downloaded by HF snapshot_download)
    output_v30/
...
os.environ["WANDB_NAME"] = "sft-v30-runpod"
...
DATA_PATH = os.path.join(WORKSPACE, "data", "training_v30.jsonl")
OUTPUT_DIR = os.path.join(WORKSPACE, "output_v30")
...
print(f"\n{'='*60}")
print(f"SFT v30 (RunPod) — Mixed v22 verbose + v29 templated, {NUM_EPOCHS} epochs")
print(f"{'='*60}")
