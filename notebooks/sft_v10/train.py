"""
SFT v10b — Fixed OOM + dynamic padding + faster training
9500 puzzle quality CoT + 2500 broad reasoning
v7 proven config: 4 LoRA targets, alpha=64, rank=32
"""
...
try:
    ...
    try:
        ...
        wandb.init(project="nemotron-reasoning", name="sft-v10b-fixed",
            config={"method": "SFT", "lora_rank": 32, "lora_alpha": 64,
                    "lr": 2e-4, "epochs": 3, "max_seq_len": 1536,
                    "data": "combined", "targets": "4-modules"},
            tags=["sft", "v10b"],
        ...
        wandb.init(project="nemotron-reasoning", name="sft-v10b-fixed",
            config={"method": "SFT"}, tags=["sft", "v10b"])
        WANDB_ACTIVE = True
...
LORA_RANK = 32
LORA_ALPHA = 64
MAX_SEQ_LEN = 1536  # Reduced from 2048 to save memory + speed
NUM_EPOCHS = 3
LR = 2e-4
BATCH_SIZE = 1
GRAD_ACCUM = 16  # Increased from 8 to save memory
...
combined_paths = (
    glob.glob("/kaggle/input/nemotron-training-v10/*.jsonl") +
    glob.glob("/kaggle/input/nemotron-training-v10/**/*.jsonl", recursive=True)
)
print(f"Found combined paths: {combined_paths}")

all_examples = []

if combined_paths:
    fpath = combined_paths[0]
    print(f"Loading combined dataset: {fpath}")
    with open(fpath) as f:
        for line in f:
            all_examples.append(json.loads(line))
    print(f"Loaded {len(all_examples)} combined examples")

if len(all_examples) < 100:
    print("Combined dataset not found or too small, generating from train.csv...")
    all_examples = []
    possible_paths = (
    ...
    def make_cot(prompt, answer, cat):
        if cat == 'gravity':
            return f"<think>\nThis is a falling distance problem. Using d = 0.5*g*t^2, I can find g from the examples.\n\nLet me calculate g from each example pair, then use the average to solve for the target.\n\nAfter computing: the answer is {answer}\n</think>\n\n\\boxed{{{answer}}}"
        elif cat == 'unit':
            return f"<think>\nUnit conversion problem. I need to find the conversion factor from the examples.\n\nDividing output by input for each pair to find the factor, then applying it to the target.\n\nResult: {answer}\n</think>\n\n\\boxed{{{answer}}}"
        elif cat == 'numeral':
            return f"<think>\nRoman numeral conversion.\nI=1, V=5, X=10, L=50, C=100, D=500, M=1000\nSubtractive: IV=4, IX=9, XL=40, XC=90, CD=400, CM=900\n\nConverting to get: {answer}\n</think>\n\n\\boxed{{{answer}}}"
        elif cat == 'cipher':
            return f"<think>\nSubstitution cipher. Building letter mapping from examples by aligning encrypted and decrypted text character by character.\n\nApplying the mapping to decrypt the target text.\n\nResult: {answer}\n</think>\n\n\\boxed{{{answer}}}"
        elif cat == 'bit':
            return f"<think>\nBit manipulation puzzle. Analyzing the transformation pattern from input-output pairs.\n\nTesting common operations: XOR, NOT, rotations, shifts, and their compositions.\n\nApplied to input: {answer}\n</think>\n\n\\boxed{{{answer}}}"
        else:
            return f"<think>\nSymbol transformation puzzle. Analyzing the mapping rules from examples to find the pattern.\n\nApplying the transformation rules.\n\nResult: {answer}\n</think>\n\n\\boxed{{{answer}}}"
...
# ============================================================
# TOKENIZE — NO PADDING (dynamic padding in collator)
# ============================================================
print("Tokenizing...")
...
class SFTDataset(Dataset):
    def __init__(self, examples, tokenizer, max_length):
        ...
        for item in examples:
            ...
            # Labels: mask user prompt with -100
            labels = list(ids)
...
def collate_fn(batch):
    """Dynamic padding — pad to max length in batch, not global max."""
...
print(f"Sequence lengths: min={min(lengths)}, max={max(lengths)}, avg={sum(lengths)/len(lengths):.0f}, median={sorted(lengths)[len(lengths)//2]}")
...
if WANDB_ACTIVE:
    wandb.log({"total_time_min": total_time})
    wandb.finish()
