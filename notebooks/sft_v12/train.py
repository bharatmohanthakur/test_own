"""
SFT v12 — alpha=128 (4x rank), puzzle-only TIR, lower LR
- 9,500 puzzle examples with computational CoT
- NO AIMO3 (focus on puzzle types only)
- Expanded LoRA targets (all 9 module types)
- alpha=128 (4x rank per Tina ICLR 2026 paper)
- LR=1e-4, 3 epochs, max_seq=2048
...
try:
    ...
    try:
        ...
        wandb.init(project="nemotron-reasoning", name="sft-v12-alpha128",
            config={"method": "SFT-TIR", "lora_rank": 32, "lora_alpha": 128,
                    "lr": 1e-4, "epochs": 3, "max_seq_len": 2048,
                    "data": "9500-puzzle-tir-only", "targets": "9-modules"},
            tags=["sft", "v12", "alpha128"],
        ...
        wandb.init(project="nemotron-reasoning", name="sft-v12-alpha128",
            config={"method": "SFT-TIR"}, tags=["sft", "v12", "alpha128"])
        WANDB_ACTIVE = True
...
LORA_RANK = 32
LORA_ALPHA = 128  # 4x rank (Tina ICLR 2026 recommendation)
MAX_SEQ_LEN = 2048
NUM_EPOCHS = 3
LR = 1e-4  # slightly lower with higher alpha
BATCH_SIZE = 1
GRAD_ACCUM = 16
...
# Try v11 puzzle-only JSONL dataset first
jsonl_paths = (
...
print(f"\n{'='*60}")
print(f"SFT v12 — alpha=128, puzzle TIR only")
print(f"{'='*60}")
