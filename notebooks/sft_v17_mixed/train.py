"""
SFT v17-mixed — 1500 examples (1000 puzzles + 500 math), 3 epochs
- Multi-domain data blending (CrossThink paper: +13% accuracy)
- 200 per puzzle category + 500 olympiad math reasoning
- 4 LoRA targets, alpha=64, LR=2e-4, 3 epochs
- ~3.5 hour run on RTX Pro 6000
"""
...
try:
    ...
    wandb_config = {"method": "SFT", "lora_rank": 32, "lora_alpha": 64,
                    ...
                    "data": "v17-mixed-1500-puzzle+math", "targets": "4-modules"}
    wandb_tags = ["sft", "v17", "mixed", "math", "3epoch"]
    try:
        wandb.login(key="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", relogin=True)
        wandb.init(project="nemotron-reasoning", name="sft-v17-mixed-3ep",
...
# ============================================================
# LOAD DATA — v17 mixed dataset (1000 puzzles + 500 math)
# ============================================================
print("Loading training data...")

all_examples = []

# Try pre-uploaded v17 mixed dataset
jsonl_paths = (
    glob.glob("/kaggle/input/nemotron-training-v17-mixed/training_v17_mixed.jsonl") +
    glob.glob("/kaggle/input/nemotron-training-v17-mixed/**/training_v17_mixed.jsonl", recursive=True) +
    glob.glob("/kaggle/input/*/training_v17_mixed.jsonl")
)

if jsonl_paths:
    fpath = jsonl_paths[0]
    print(f"Loading v17 mixed dataset: {fpath}")
    with open(fpath) as f:
        for line in f:
            all_examples.append(json.loads(line))
    print(f"Loaded {len(all_examples)} v17 mixed examples")

# Fallback: generate from train.csv with inline solvers
if len(all_examples) < 100:
    print("v17 mixed dataset not found, generating from train.csv with solvers...")
    possible_paths = (
...
print(f"\n{'='*60}")
print(f"SFT v17-mixed — 1500 examples (1000 puzzles + 500 math), 3 epochs")
print(f"{'='*60}")
