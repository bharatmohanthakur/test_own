"""
SFT v30 — MIXED DATA: v22 verbose CoT (504) + v29 Donald templated (500) = 1004
v29 alone scored 0.52 (dropped from v22's 0.65) because:
- missing equation_transform type (1/6 of eval)
- rigid hard-labeled steps crowded out general reasoning
- binary template only handles 1+2 input gates (58% of binary)
v30 fix: KEEP v22's verbose reasoning (covers all 6 types) + ADD v29's structured templates
This should teach the model structured patterns WITHOUT losing general ability.
Same proven config as v22: r=32, alpha=64, LR=2e-4, 3 epochs, 4 LoRA targets.
"""
...
try:
    ...
    try:
        ...
        wandb.init(project="nemotron-reasoning", name="sft-v30-mixed",
...
LORA_RANK = 32
LORA_ALPHA = 64       # v7 proven (0.69 score)
LORA_DROPOUT = 0.0    # 0 enables Unsloth fast kernels
MAX_SEQ_LEN = 4096
NUM_EPOCHS = 3        # v7 used 3 — exact match
BATCH_SIZE = 2
...
jsonl_paths = []
for pat in [
    "/kaggle/input/nemotron-training-v30/training_v30.jsonl",
    "/kaggle/input/datasets/bharatmohan/nemotron-training-v30/training_v30.jsonl",
    "/kaggle/input/*/training_v30.jsonl",
    "/kaggle/input/*/*/training_v30.jsonl",
    "/kaggle/input/*/*/*/training_v30.jsonl",
]:
...
if jsonl_paths:
    ...
    print("ERROR: training_v30.jsonl NOT FOUND")
    sys.exit(1)
...
print(f"\n{'='*60}")
print(f"SFT v30 — Mixed v22 verbose + v29 templated (1004) + Unsloth + 3 epochs")
print(f"{'='*60}")
