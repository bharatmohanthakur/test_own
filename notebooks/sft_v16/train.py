"""
SFT v16 — Clean verified CoT, 2 epochs, no equation_transform
- 7941 verified examples (dropped equation_transform — 50% buggy data)
- 4 LoRA targets (in_proj, out_proj, up_proj, down_proj) — same as v7
- alpha=64, LR=2e-4, 2 epochs, max_seq=4096
...
try:
    ...
    wandb_config = {"method": "SFT", "lora_rank": 32, "lora_alpha": 64,
                    ...
                    "data": "v16-clean-7941-no-eqtransform", "targets": "4-modules"}
    wandb_tags = ["sft", "v16", "clean", "2epoch"]
    try:
        wandb.login(key="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", relogin=True)
        wandb.init(project="nemotron-reasoning", name="sft-v16-clean-2ep",
...
jsonl_paths = (
    glob.glob("/kaggle/input/nemotron-training-v16/training_v16.jsonl") +
    glob.glob("/kaggle/input/nemotron-training-v16/**/training_v16.jsonl", recursive=True) +
    glob.glob("/kaggle/input/*/training_v16.jsonl")
)

if jsonl_paths:
    fpath = jsonl_paths[0]
    print(f"Loading v16 clean dataset: {fpath}")
    with open(fpath) as f:
        for line in f:
            all_examples.append(json.loads(line))
    print(f"Loaded {len(all_examples)} v16 examples")

# Fallback: generate from train.csv with inline solvers
if len(all_examples) < 100:
    print("v16 dataset not found, generating from train.csv with solvers...")
    possible_paths = (
...
print(f"\n{'='*60}")
print(f"SFT v16 — Clean verified CoT, 2 epochs")
print(f"{'='*60}")
