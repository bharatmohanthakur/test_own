"""
GRPO v28 — Load v22 SFT adapter, GRPO on top
v28 fix: monkey-patch Unsloth's chunked_hidden_states_selective_log_softmax
to detect when input is already logits (Nemotron-3-Nano returns .logits with vocab_size dim,
not hidden states with hidden_size dim) and fall back to chunked_selective_log_softmax.
v27 v4 errored with: RuntimeError: mat1 and mat2 shapes cannot be multiplied (154x131072 and 2688x131072)
because Unsloth's text-only branch (line 2671) lacks the guard that the VLM branch (line 2700) has.
...
print(f"\n{'='*60}")
print(f"GRPO v28 — SFT adapter loaded, GRPO only")
print(f"{'='*60}")
