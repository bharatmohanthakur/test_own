...
subprocess.run("pip install -q --no-index --find-links /kaggle/input/nemotron-packages/packages unsloth trl peft transformers datasets accelerate bitsandbytes 2>/dev/null || pip install -q --no-index --find-links /kaggle/input/datasets/mayukh18/nemotron-packages/packages unsloth trl peft transformers datasets accelerate bitsandbytes 2>/dev/null", shell=True)
for w in glob.glob("/kaggle/input/nemotron-packages/causal_conv1d*.whl") + glob.glob("/kaggle/input/nemotron-packages/mamba_ssm*.whl") + glob.glob("/kaggle/input/datasets/mayukh18/nemotron-packages/causal_conv1d*.whl") + glob.glob("/kaggle/input/datasets/mayukh18/nemotron-packages/mamba_ssm*.whl"):
...
with open(TRAIN_CSV) as fh:
    for row in csv.DictReader(fh):
        grpo_examples.append({"prompt": row["prompt"], "answer": row["answer"]})
random.seed(RANDOM_SEED)
random.shuffle(grpo_examples)
grpo_examples = grpo_examples[:500]
print(f"Loaded {len(grpo_examples)} prompts for GRPO")
...
# Load v42 SFT adapter on top of base model
print(f"Loading v42 SFT adapter from {ADAPTER_PATH} ...")
