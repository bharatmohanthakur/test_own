"""
SFT Fast Dev — mixed v11 TIR, short-turnaround config
- Prefer combined_v11_tir.jsonl (22k mixed examples: puzzle + broad reasoning)
- Keep all broad-reasoning examples and sample the remainder for faster experiments
- 4 LoRA targets (in_proj, out_proj, up_proj, down_proj)
- alpha=64, LR=1e-4, 1 epoch, max_seq=2048
...
try:
    import wandb
    # Kaggle competition runs frequently block internet. Force offline mode to avoid
    # long login/init stalls before training even starts.
    os.environ["WANDB_MODE"] = "offline"
    wandb.init(project="nemotron-reasoning", name="sft-fast-dev",
        config={"method": "SFT-TIR", "lora_rank": 32, "lora_alpha": 64,
                "lr": 1e-4, "epochs": 1, "max_seq_len": 2048,
                "data": "mixed-v11-tir-capped", "targets": "4-modules",
                "max_examples": 6000},
        tags=["sft", "fast-dev", "mixed", "2048"],
...
MAX_EXAMPLES = 6000
RANDOM_SEED = 42
BROAD_CATEGORIES = {"math", "logic", "science", "code", "general"}
...
# Prefer the mixed v11 file first so we keep broader reasoning coverage.
jsonl_paths = (
    glob.glob("/kaggle/input/nemotron-training-v11/combined_v11_tir.jsonl") +
    glob.glob("/kaggle/input/nemotron-training-v11/**/combined_v11_tir.jsonl", recursive=True) +
    glob.glob("/kaggle/input/*/combined_v11_tir.jsonl") +
    glob.glob("/kaggle/input/*/**/combined_v11_tir.jsonl", recursive=True) +
    glob.glob("/kaggle/input/nemotron-training-v11/training_data_v11.jsonl") +
    glob.glob("/kaggle/input/nemotron-training-v11/**/training_data_v11.jsonl", recursive=True) +
    glob.glob("/kaggle/input/nemotron-training-v11/puzzle_data_v11.jsonl") +
    glob.glob("/kaggle/input/nemotron-training-v11/**/puzzle_data_v11.jsonl", recursive=True) +
...
# Prefer the mixed file, then broader training_data, then puzzle-only as fallback.
fpath = None
for p in jsonl_paths:
    name = os.path.basename(p)
    if name == "combined_v11_tir.jsonl":
        fpath = p
        break
if fpath is None:
    for p in jsonl_paths:
        if os.path.basename(p) == "training_data_v11.jsonl":
            fpath = p
            break
if fpath is None:
    for p in jsonl_paths:
        if 'puzzle' in p:
            fpath = p
            break
if fpath is None and jsonl_paths:
    fpath = jsonl_paths[0]
...
if fpath:
    ...
    if len(all_examples) > MAX_EXAMPLES:
        broad_examples = [ex for ex in all_examples if ex.get("category") in BROAD_CATEGORIES]
        other_examples = [ex for ex in all_examples if ex.get("category") not in BROAD_CATEGORIES]
        rng = random.Random(RANDOM_SEED)
        rng.shuffle(other_examples)
        keep_other = other_examples[:max(0, MAX_EXAMPLES - len(broad_examples))]
        all_examples = broad_examples + keep_other
        rng.shuffle(all_examples)
        print(
            f"Capped dataset to {len(all_examples)} examples "
            f"({len(broad_examples)} broad + {len(keep_other)} other)"
        )
...
# ============================================================
# APPLY LoRA — 4 TARGETS FOR FASTER ITERATION
# ============================================================
print("Setting up LoRA with 4 fast-dev targets...")
...
print(f"\n{'='*60}")
print(f"SFT Fast Dev — alpha=64, mixed v11 TIR, 2048 context")
print(f"{'='*60}")
