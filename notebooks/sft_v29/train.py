"""
SFT v29 — Donald Galliano playbook templated training data
400 examples (100 each of gravity, unit, roman, cipher) with hard-labeled steps:
- gravity: rate-first SOLVE + dual-rate VER + X.XX format
- unit: rate-first SOLVE + tighter VER (0.01) + X.XX format
- roman: DECOMPOSE + incremental CAT + round-trip VER
- cipher: LEN + TABLE + char-by-char DECRYPT + VOCAB fill (77-word vocab) + CHECK
Same proven config as v22 (which scored 0.69 under old metric):
- LoRA r=32 alpha=64, LR=2e-4, 3 epochs, targets: in_proj/out_proj/up_proj/down_proj
"""
...
try:
    ...
    try:
        ...
        wandb.init(project="nemotron-reasoning", name="sft-v29-donald-templated",
...
if jsonl_paths:
    ...
    print("ERROR: training_v29.jsonl NOT FOUND")
    sys.exit(1)
...
print(f"\n{'='*60}")
print(f"SFT v29 — Donald playbook templated data (400) + Unsloth + 3 epochs")
print(f"{'='*60}")
