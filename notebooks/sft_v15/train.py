"""
SFT v15 — High-quality solved CoT, v7-style config (our best: 0.69)
- 9496 verified puzzle examples with computational CoT
- 4 LoRA targets (in_proj, out_proj, up_proj, down_proj) — same as v7
- alpha=64, LR=2e-4, 1 epoch, max_seq=4096
...
try:
    ...
    wandb.init(project="nemotron-reasoning", name="sft-v15-solved-cot",
        config={"method": "SFT", "lora_rank": 32, "lora_alpha": 64,
                "lr": 2e-4, "epochs": 1, "max_seq_len": 4096,
                "data": "v15-solved-verified-9496", "targets": "4-modules"},
        tags=["sft", "v15", "solved-cot"],
...
jsonl_paths = (
    glob.glob("/kaggle/input/nemotron-training-v15/training_v15.jsonl") +
    glob.glob("/kaggle/input/nemotron-training-v15/**/training_v15.jsonl", recursive=True) +
    glob.glob("/kaggle/input/*/training_v15.jsonl")
)

if jsonl_paths:
    fpath = jsonl_paths[0]
    print(f"Loading v15 solved dataset: {fpath}")
    with open(fpath) as f:
        for line in f:
            all_examples.append(json.loads(line))
    print(f"Loaded {len(all_examples)} v15 examples")

# Fallback: generate from train.csv with inline solvers
if len(all_examples) < 100:
    print("v15 dataset not found, generating from train.csv with solvers...")
    possible_paths = (
...
print(f"\n{'='*60}")
print(f"SFT v15 — Solved CoT, v7-style config")
print(f"{'='*60}")
