"""
SFT v18 — 1699 examples (1200 puzzles + 499 math), 2 epochs
- 200 per ALL 6 puzzle categories (including equation_transform)
- GF(2) solver CoT for bit_ops, answer-extended cipher, structured eq_transform
- 499 math reasoning (AIMO3 olympiad) for generalization
- 4 LoRA targets, alpha=64, LR=2e-4, 2 epochs
...
try:
    ...
    wandb_config = {"method": "SFT", "lora_rank": 32, "lora_alpha": 64,
                    "lr": 2e-4, "epochs": 2, "max_seq_len": 4096,
                    "data": "v18-2000-gf2-math", "targets": "4-modules"}
    wandb_tags = ["sft", "v18", "gf2-solver", "math", "2epoch"]
    try:
        wandb.login(key="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", relogin=True)
        wandb.init(project="nemotron-reasoning", name="sft-v18-2ep",
...
# Try pre-uploaded v18 dataset first
jsonl_paths = (
    glob.glob("/kaggle/input/nemotron-training-v18/training_v18.jsonl") +
    glob.glob("/kaggle/input/nemotron-training-v18/**/training_v18.jsonl", recursive=True) +
    glob.glob("/kaggle/input/*/training_v18.jsonl")
)

if jsonl_paths:
    fpath = jsonl_paths[0]
    print(f"Loading v18 dataset: {fpath}")
    with open(fpath) as f:
        for line in f:
            all_examples.append(json.loads(line))
    print(f"Loaded {len(all_examples)} v18 examples")

# Fallback: generate from train.csv with inline solvers
if len(all_examples) < 100:
    print("v18 dataset not found, generating from train.csv with solvers...")
    possible_paths = (
...
print(f"\n{'='*60}")
print(f"SFT v18 — 2000 examples (1500 puzzles + 500 math), 2 epochs")
print(f"{'='*60}")
